                ...
                if hasattr(members, '__aiter__'):  # Check if it's an async generator
                    async for member in members:
                        try:
                            ...
                            await client.add_chat_members(chat_id=Var.CB_CHANNEL, user_ids=[member.user.id])  # Add member to CB_CHANNEL
                            await asyncio.sleep(0.5)
                    ...
                    for member in members:
                        try:
                            await X.forward(member.user.id)
                            await client.add_chat_members(chat_id=Var.CB_CHANNEL, user_ids=[member.user.id])
                            await asyncio.sleep(0.5)
                        except FloodWait as e:
                            print(f"Sleeping for {str(e.x)}s")
                            await asyncio.sleep(e.x)
